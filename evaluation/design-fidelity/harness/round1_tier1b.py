"""Round 1, Tier-1b SCREEN use #1 — hill-climb campaign, design-fidelity mj-reviewer lens.

Evaluates 3 challenger variants (harness/arms_variants/{V1_MVI,V2_LEAN,V3_COSHYBRID}.md)
against the champion baseline, PAIRED per scenario, on the frozen 12 scenarios, using the
PRIMARY model per the amended protocol (see ../HILL-CLIMB-PLAN.md AMENDMENT + ledger
round-0 protocol_amendment): claude-fable-5 ONLY.
...
arm = the variant code (V1_MVI / V2_LEAN / V3_COSHYBRID), which is what keeps these
...
Outputs:
  runs/round1_tier1b/round1_raw.jsonl          (108 fresh generations)
  runs/round1_tier1b/round1_extractions.jsonl  (108 gpt-4.1 extractions, audit trail)
  runs/round1_tier1b/round1_results.json       (all numbers)
  ../hillclimb/ROUND1.md                        (writeup)
  ../hillclimb/LEDGER.jsonl                     (append: 1 entry/variant + round decision)

Usage:
    python round1_tier1b.py
"""
...
OUT_DIR = RUNS_DIR / "round1_tier1b"
RAW_PATH = OUT_DIR / "round1_raw.jsonl"
EXTRACT_PATH = OUT_DIR / "round1_extractions.jsonl"
RESULTS_PATH = OUT_DIR / "round1_results.json"
...
ROUND1_MD_PATH = HILLCLIMB_DIR / "ROUND1.md"
...
def extract_one(sid: str, tag: str, sample: int, raw_text: str) -> dict:
    ...
    key = cache.make_key(
        "round1_design_extract_v1", CAMPAIGN_JUDGE_MODEL, sid, tag, sample, user
    )
    cached = cache.get(key, cache_dir=CACHE_DIR)
    if cached is None:
        ...
        cache.put(
            ...
            cached,
            meta={
                "stage": "round1_design_extract_v1",
                "scenario_id": sid,
                "tag": tag,
                "sample": sample,
            },
...
def concern_match(sid: str, tag: str, mj_c: str | None, arm_c: str | None) -> int:
    ...
    key = cache.make_key(
        "round1_design_concern_grade_v1", CAMPAIGN_JUDGE_MODEL, sid, tag, 0, user
    )
    cached = cache.get(key, cache_dir=CACHE_DIR)
    if cached is None:
        cached = llm.call_openai(
        ...
        cache.put(
            ...
            cached,
            meta={
                "stage": "round1_design_concern_grade_v1",
                "scenario_id": sid,
                "tag": tag,
            },
            cache_dir=CACHE_DIR,
...
def main() -> int:
    ...
    results = {
        "campaign": "round1_tier1b_screen_1",
        "tier": "1b",
        "round": 1,
