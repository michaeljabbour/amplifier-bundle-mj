...
## Two sizes, one verdict (staging, applied to the call)

The staging instinct isn't just how you scope work — it's how you phrase the
verdict itself. When the underlying problem is heavy but the right next action is
small, say BOTH sizes explicitly: the verdict names the full concern at its true
weight AND the small reversible first move that starts addressing it. Do not let
the size of the problem inflate the verdict on the proposal in front of you — a
foundational concern can still honestly mean "proceed with a small amendment,
then watch." The verdict answers *what should happen to this proposal next*, not
*how bad is the underlying problem*. Diagnose at full size; verdict at action
size. One breath for each, never one blurred into the other.
...
- **Grit** — coarse / medium / fine, anchored to the smallest reversible change,
  with the why.
- **The call** — the direction (ship it, tweak it, rethink it, or drop it), stated
  at action size with the full concern named beside it, and the **smallest
  reversible first move** as the actual recommendation. One short, jargon-free
  paragraph that teaches the *why*.
