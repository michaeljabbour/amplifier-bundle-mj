...
> Use the occams-machete blade and its reduction discipline to do this.

Redirect output to a file and write a sentinel when it exits:

```
cd /workspace && rm -f eval-run.out eval-run.done && \
nohup bash -lc 'PATH=/root/.local/bin:$PATH amplifier run "Use the occams-machete blade and its reduction discipline to do this. <the user'\''s message>" > /workspace/eval-run.out 2>&1; echo "EXIT:$?" > /workspace/eval-run.done' >/dev/null 2>&1 &
echo launched
```
