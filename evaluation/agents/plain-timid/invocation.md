...
> Be conservative: only remove code you are certain is dead or unused. When in doubt, leave it untouched. Never risk changing behavior.

Redirect output to a file and write a sentinel when it exits:

```
cd /workspace && rm -f eval-run.out eval-run.done && \
nohup bash -lc 'PATH=/root/.local/bin:$PATH amplifier run "Be conservative: only remove code you are certain is dead or unused. When in doubt, leave it untouched. Never risk changing behavior. <the user'\''s message>" > /workspace/eval-run.out 2>&1; echo "EXIT:$?" > /workspace/eval-run.done' >/dev/null 2>&1 &
echo launched
```
